% © Copyright IBM Corporation 2001, 2026
...
\documentclass[USenglish,fontsize=11pt,twoside,%
  toc=bib,toc=idx,toc=graduated,numbers=noendperiod,
  % -- Hyperref --
  bookmarks=true,
...
\KOMAoptions{
  paper=letter,
  DIV=13,BCOR=1cm,
  % If you prefer inter-paragraph spacing instead of indentation:
  %
  % parskip=half
}

\DeclareTOCStyleEntry[numwidth=3.2em]{default}{figure}
\DeclareTOCStyleEntry[numwidth:=figure]{default}{table}
\DeclareTOCStyleEntry[level=0,indent=0pt,numwidth:=figure]{default}{lstlisting}
...
\lowertitleback{%
  ...
  \noindent © Copyright IBM Corporation 2001, 2026
...
\listoffigures
\listoftables
\newcommand*{\listoflolname}{List of Listings}
\lstlistoflistings
...
\begin{description}
\item[1.7] Describe \texttt{\_Float16} and \texttt{\_BitInt(N)}; add
  information related to DWARF CFI and the frame pointer; document a
  deviation from the ELF hash table format.  Published at
  \url{https://github.com/ibm/s390x-abi}, June 2025.
...
\begin{table}
  \centering
  \index{\texttt{\_\_int128}}%
  \index{\texttt{\_Bool}}%
  \index{\texttt{\_Float16}}%
  \index{\texttt{\_Decimal32}}%
  \index{\texttt{\_Decimal64}}%
  \index{\texttt{\_Decimal128}}%
  \begin{DIFnomarkup}
  \begin{threeparttable}
    \begin{tabularx}{\textwidth}{XlrrX}
      ...
      & \texttt{unsigned \_\_int128}\tnote{\ddagger} & 16 & 8 \\
      \midrule
      ...
      & \texttt{long long} & 8 & 8 \\
      & \texttt{\_\_int128}\tnote{\ddagger} & 16 & 8 \\
      & \texttt{signed \_\_int128}\tnote{\ddagger} & 16 & 8 \\
      \midrule
      ...
      \texttt{\_Float16}\tnote{\ddagger} & 2 & 2 & tiny BFP \\
      ...
      \texttt{\_Decimal32}\tnote{\ddagger} & 4 & 4 & short DFP \\
      & \texttt{\_Decimal64}\tnote{\ddagger} & 8 & 8 & long DFP \\
      & \texttt{\_Decimal128}\tnote{\ddagger} & 16 & 8 & extended DFP \\
      \bottomrule
    \end{tabularx}
    \medskip
    \begin{tablenotes}
    ...
    \item [\ddagger] These types are an extension to C (ISO/IEC
      9899:2011).
    \end{tablenotes}
  \end{threeparttable}
  \end{DIFnomarkup}
...
\subsubsection{Bit-Precise Integers \texttt{\_BitInt(\(N\))}}
\index{N-bit integer}%
\index{\texttt{\_BitInt}}%
\index{bit-precise integer}%
For an integer constant \(N\) the bit-precise signed \(N\)-bit integer
type \texttt{\_BitInt(\(N\))} and its unsigned counterpart
\texttt{unsigned \_BitInt(\(N\))} are represented as follows:
\begin{itemize}
\item If \(N \le \NBITS{}\): like the smallest appropriately-signed
  fundamental integer type that covers its value range.  Thus the size and
  alignment is 1, 2, {\ifzseries 4, or 8\else or 4\fi} bytes.
\item If \(N > \NBITS{}\): like a struct of \texttt{long} or
  \texttt{unsigned long} fields, using the smallest number of fields that
  can contain the type.  The fields are ordered from most to least
  significant, and the least significant bit is stored in the least
  significant bit of the last field.
\end{itemize}
In both cases the representation may have more bits than the abstract
type.  Excess bits use sign or zero extension, as appropriate.

\Cref{tab:bitint} summarizes the representation of these integer types.

\begin{table}
  \centering
  \begin{DIFnomarkup}
  \begin{tabular}[t]{c@{\,\ldots\,}ccc}
    \toprule
    \multicolumn{2}{c}{\(N\)} & Size & Alignment \\
    \midrule
     1 &   8 &  1 & 1 \\
     9 &  16 &  2 & 2 \\
    17 &  32 &  4 & 4 \\
    ...
    33 &  64 &  8 & 8 \\
    \fi
    \multicolumn{2}{c}{\(>\NBITS{}\)}
                              & \(\NBYTES{}\cdot\left\lceil\frac{N}{\NBITS{}}\right\rceil\)
                                     & \NBYTES{} \\
    \bottomrule
  \end{tabular}
  \end{DIFnomarkup}
  \caption[Bit-Precise Integer Types] {Bit-Precise Integer Types
    \texttt{\_BitInt(\(N\))} and \texttt{unsigned \_BitInt(\(N\))}}
  \label{tab:bitint}
\end{table}
...
\begin{table}
  ...
  \begin{threeparttable}
    \begin{tabularx}{\textwidth}{lXl}
      ...
      \texttt{r7}…\texttt{r10} & -- & Saved \\
      \texttt{r11} & (Commonly used as frame pointer) & Saved \\
    ...
    \item [\ddagger] Except for bytes 0--7, which are aliased to
...
\item[Return value:] \index{return value!register} When a called function
  ...
  \cref{retvalues}.
\item[Frame pointer:] \index{frame pointer} A frame pointer register
  establishes consistent addressability of a function's local variables
  when dynamic stack allocation is performed, see \cref{dynamicstack}.  If
  a frame pointer is maintained, it should be stored in \texttt{r11}.
...
\item General registers \texttt{r2} to \texttt{r5} should be second
  choice.
\item General registers \texttt{r11} to \texttt{r15} should only be
...
\item[\jumplabel{scan}:] If there are no more arguments, terminate.
  ...
  \item[\jumplabel{double\_or\_float}:] A double\_or\_float is
    ...
    \item A structure equivalent to one of the above.  A structure is
      ...
      type~$T$.
    \end{itemize}
    If $\mbox{\texttt{fr}}>{\ifzseries 6\else 2\fi}$, that is, if there
    are no more floating-point registers available for parameter passing,
    go to \jumplabel{more}.  Otherwise, load the argument value
    left-aligned into floating-point register \texttt{fr}, set \texttt{fr}
    to $\mbox{\texttt{fr}}+2$, and go to \jumplabel{scan}.
  ...
  \item[\jumplabel{simple\_arg}:] A simple\_arg is one of the following:
    ...
    \item One of the simple integer types no more than \NBITS{} bits wide.
      ...
      \texttt{int}, \texttt{long}, \ifzseries \texttt{long} \texttt{long},
      \fi \texttt{\_BitInt(\(N\))} with \(N \le \NBITS{}\), their unsigned
      counterparts, \texttt{\_Bool}, and any \texttt{enum} type.  If such
      an argument is shorter than \NBITS{} bits, replace it by a full
      \NBITS{}-bit integer representing the same number, using sign or
      zero extension, as appropriate.
    ...
    \item A struct or a union of 1, 2, {\ifzseries 4, or 8\else or 4\fi}
      ...
      on the left.
    \item A struct or union of any other size, any integer type with more
      than \NBITS{} bits, a \texttt{long} \texttt{double}, a
      \texttt{\_Decimal128}, a complex type, or a vector whose size
      exceeds 16 bytes.  Replace such an argument by a pointer to the
      object, or to a copy where necessary to enforce call-by-value
      semantics.  Only if the caller can ascertain that the object is
      ``constant'' can it pass a pointer to the object itself.
    \end{itemize}
...
\item[\jumplabel{more}:] The argument cannot be passed in registers; it
  ...
  explained as appropriate, the argument now has a size of 2, 4, or 8
  bytes.
  \\
  Copy the argument value right-aligned into the \NBYTES{}-byte parameter
  slot at the current stack position \texttt{starg}, leaving the skipped
  bytes (if any) at unspecified values.  Increment \texttt{starg} by
  \NBYTES{}, then go to \jumplabel{scan}.
  \ifzseries\else Or if the argument has 8 bytes, copy it to the next two
    parameter slots and increment \texttt{starg} by 8 instead.\fi
...
\item A value of type \texttt{\_Float16} is returned in the leftmost two
  bytes of \texttt{f0} and encoded in tiny BFP format.  The remaining
  bytes of \texttt{f0} are unspecified.
\item Any integer type with \NBITS{} or fewer bits, including
  \texttt{\_BitInt(\(N\))} with \(N \le \NBITS{}\), \texttt{\_Bool}, as well as
  any \texttt{enum} type, is returned in \texttt{r2}.  The return value is
  zero- or sign-extended to \NBITS{} bits, as appropriate.
...
\item Any other type, such as \texttt{long} \texttt{double},
  \texttt{\_Decimal128}, \texttt{\_\_int128}, \texttt{\_BitInt(\(N\))}
  with \(N > \NBITS{}\), a complex type, a structure, a union, or a vector
  larger than 16 bytes, is returned in a return value buffer allocated by
  the caller.  This buffer's address is treated like a ``hidden argument''
  and passed by the caller in \texttt{r2}.
\end{itemize}
...
in either direction).  {\ifzseries For large functions, another
  self-relative jump is available with a range of 8$\,$Gbytes (up to
  4$\,$Gbytes in either direction).\fi}%
...
The register number mapping\index{DWARF!register
...
\subsection{DWARF Locations}
\index{DWARF!locations}
...
\subsection{Canonical Frame Address}
\index{canonical frame address}
\index{CFA}
According to the DWARF standard, each call frame is identified by an
address on the stack referred to as the canonical frame address or CFA.
Let \texttt{SP} be the value of the stack pointer at the call site in the
previous frame.  Then the CFA is defined to have the value
\(\mbox{\texttt{SP}} + \STACKSIZE\) for \ARCH{} processors.

\subsection{Call Frame Information}
\index{DWARF!call frame information}%
When interpreting DWARF call frame information, the default rule for the
following registers is the same value rule:
\begin{itemize}
\item \texttt{r6} through \texttt{r15}
\item \texttt{v8} through \texttt{v15}
\item \texttt{a0} and \texttt{a1}
\end{itemize}
For all other registers, the undefined rule applies by default.
...
\ifdefined\ifHtml
  \begin{table}
    \centering
    \caption{Relocation types\label{tab:relocations}}
  \end{table}
\fi
...
\label{hashtable}
\index{hash table} Each loadable object may contain a hash table to speed
up symbol access.  ELF specifies a hash table format that consists of two
arrays of symbol table indexes, preceded by each of these array's number
of entries.

While the general ELF format definition specifies all integers in the hash
table to be stored as \texttt{Elf\NBITS\_Word}, the \ABINAME{} ABI uses
\texttt{Elf\NBITS\_Xword} instead.
